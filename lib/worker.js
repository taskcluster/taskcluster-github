...
worker.webHookHandler = async function(message, context) {
  ...
  try {
    ...
    taskclusterConfig = yaml.load(taskclusterConfig);
    ...
    if (!isCollaborator) {
      let orgs = await context.githubAPI.users(login
      ).orgs.fetch();
      isCollaborator = utils.listContainsExpressions(message.payload.organization,
        orgs.map((org) => {return org.login}));
    }
    ...
    // If we have insecure tasks, a non-collaborator can still run jobs
    let hasInsecureTask = false;
    for (taskIndex in taskclusterConfig.tasks) {
      if (taskclusterConfig.tasks[taskIndex].task.extra.insecure == true) {
        hasInsecureTask = true;
        break;
      }
    }

    // If all of the collaborator checks fail and there are no insecure tasks we
    // should post to the commit and ignore the request
    if (!isCollaborator && !hasInsecureTask) {
      let msg = `@${login} does not have permission to trigger any tasks.`;
