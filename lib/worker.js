...
worker.webHookHandler = async function(message, context) {
  ...
  try {
    ...
    // Decide if a user has permissions to run secure tasks.
    ...
    if (graphConfig.tasks.length) {
      ...
      let error = new Error(`@${login} does not have permission to trigger any tasks.`);
      throw error;
    } else {
    ...
    debug(e);
    let errorMessage = 'TaskCluster, ' + e.message;
