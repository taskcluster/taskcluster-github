...
api.declare({
  ...
  // We locate valid publishers by naming convention
  // and fail if we don't find one which matches the
  // event type
  ...
  let webhookSecret = this.cfg.get('webhook_secret')
  ...
  let organizationName = body.organization.login
  let repositoryName = body.repository.full_name
  let action = body.action
  let details = {}
  await publisher({organizationName, repositoryName, action, details});
