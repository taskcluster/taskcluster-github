 ...
 *  params {
 *    taskclusterConfig:  '...', a raw YAML taskcluster config
 *    payload:            {},    GitHub WebHook message payload
 *    validator:          {}     A taskcluster.base validator instance
 *    isCollaborator      bool,  The permission status of the user
  ...
  return new Promise(function(accept, reject) {
    try {
      ...
      taskclusterConfig.tasks = taskclusterConfig.tasks.map((taskConfig) => {
        ...
        // being handled
        let github = taskConfig.task.extra.github;
        // nothing will trigger a task with no extra.github section
        if (!github) return false;
        let events = github.events;
        if (!utils.listContainsExpressions(events, [payload.details['event.type']])) return false;
        // non-collaborators can only trigger explicitly insecure tasks
        if (!params.isCollaborator && !github.insecure) return false;
        return true;
      });
