...
class Handlers {
  ...
  async setup(options) {
    ...
    this.groupStatusPq = await consume(
      {
        client: this.pulseClient,
        bindings: groupStatusBindings,
        queueName: this.statusQueueName,
      },
      this.monitor.timedHandler('groupStatuslistener', callHandler('status', groupStatusHandler).bind(this))
    );
  }
