...
class Handlers {
  ...
  async setup() {
    ...
    this.statusListener.bind(schedulerEvents.taskGraphRunning(route));
    this.statusListener.bind(schedulerEvents.taskGraphBlocked(route));
    this.statusListener.bind(schedulerEvents.taskGraphFinished(route));

    this.jobListener.on('message', this.monitor.timedHandler('joblistener', this.jobHandler));
    this.statusListener.on('message', this.monitor.timedHandler('statuslistener', this.statusHandler));
  ...
  async jobHandler(message) {
    ...
    debug(`handling ${message.payload.details['event.type']} webhook for: ${organization}/${repo}`);
